...
from seabiscuit import Seabiscuit
from seabiscuit_client import SeabiscuitClient
from seabiscuit_master import SeabiscuitMaster, GENERATOR_PREFIX
...
def create_parser():
    def add_config_file(grp, is_required):
        grp.add_argument(
            ...
            help='YAML configuration file, e.g. configurations/seabiscuit-gan/celeba.yml. Only required on master.')

    parser = CustomArgumentParser(description='Seabiscuit - coevolutionary learning for neural networks')
    parser.add_argument(
        '--cuda-device',
        type=int,
        required=False,
    ...
    subparsers = parser.add_subparsers(dest='task', help='Seabiscuit task to run')
...
def calc_score(args, cc):
    ...
    SeabiscuitMaster().save_samples(dataset, output_dir, dataloader)
...
def generate_samples(args, cc):
    ...
    dataset = MixedGeneratorDataset(population,
                                    ...
                                    cc.settings['trainer']['mixture_generator_samples_mode'])
    os.makedirs(output_dir, exist_ok=True)
    SeabiscuitMaster().save_samples(dataset, output_dir, dataloader, sample_size, batch_size)
    ...
    if args.task == 'train':
        if args.distributed:
            if args.master:
                ...
                SeabiscuitMaster().run()
            elif args.client:
                SeabiscuitClient().run()
        else:
            cc = initialize_settings(args)
            seabiscuit = Seabiscuit()
            seabiscuit.run(cc.settings['trainer']['n_iterations'])
