...
class Seabiscuit:
    """
    Seabiscuit is a toolkit that trains generative adversarial networks with coevolutionary methods.

    For more details about its usage, visit the GitHub page:
    https://github.com/seabiscuit2018/seabiscuit-gan
    """
    ...
    def __init__(self, trainer=None):
        ...
        if is_cuda_available() and self.cc.settings['trainer']['params']['score']['cuda']:
            self._logger.info("CUDA is supported on this device and will be used.")
        elif is_cuda_available() and (not self.cc.settings['trainer']['params']['score']['cuda']):
        ...
        set_random_seed(final_seed, self.cc.settings['trainer']['params']['score']['cuda'])
    ...
    def run(self, n_iterations, stop_event=None):
        ...
        (generator, g_loss), (discriminator, d_loss), all_net_weights_dist \
            = self.trainer.train(n_iterations, stop_event)
        ...
        # Save the distances between neural network weights
        if all_net_weights_dist is not None:
            np.save(os.path.join(self.cc.output_dir, 'all_net_weights_dist.npy'), all_net_weights_dist)
