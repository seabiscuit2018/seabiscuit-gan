...
class ClientAPI:
    ...
    def run_experiment():
        ...
        worker_thread = Thread(target=ClientAPI._run_seabiscuit, args=(config,))
        worker_thread.start()
    ...
    def _run_seabiscuit(config):
        ...
        try:
            ...
            seabiscuit.run(cc.settings['trainer']['n_iterations'], ClientAPI._stop_event)
            ...
            ClientAPI.is_finished = True
            ClientAPI._logger.critical('An unhandled error occured while running Seabiscuit: {}'.format(ex))
