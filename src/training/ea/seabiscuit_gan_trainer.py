...
class SeabiscuitGANTrainer(EvolutionaryAlgorithmTrainer):
    """
    ...
    def __init__(self, dataloader, network_factory, population_size=10, tournament_size=2, mutation_probability=0.9,
        ...
        if self.cc.settings['trainer'].get('calculate_net_weights_dist', calculate_net_weights_dist):
            self.all_net_weights_dist = []
        else:
            self.all_net_weights_dist = None
    ...
    def train(self, n_iterations, stop_event=None):
        ...
        for iteration in range(n_iterations):
            ...
            # Calculate l2_distance between all individuals for every iteration
            if self.all_net_weights_dist is not None:
                self.all_net_weights_dist.extend(all_generators.net_weights_dist)
                self.all_net_weights_dist.extend(all_discriminators.net_weights_dist)
